...
/**
 * @swagger
 * /partidos/mis-asistencias:
 *   get:
 *     summary: Listar partidos futuros donde el jugador confirmó asistencia
 *     description: Retorna los partidos a los cuales el jugador asistirá basándose en la tabla asistencias.
 *     tags:
 *       - Partidos
 *     parameters:
 *       - in: query
 ...
 *     responses:
 *       200:
 *         description: Lista de partidos confirmados
 *         content:
 *           application/json:
 *             schema:
 *               type: object
...
export const handlerLocal = async (event) => {
    try {
        ...
        // Fecha actual para filtrar solo partidos futuros
        const ahora = new Date().toISOString();

        // Consulta invertida: desde partidos con JOIN a asistencias
        // Esto permite ordenar correctamente por partidos.fecha
        ...
        if (error) {
            ...
            return {
                statusCode: 500,
                body: JSON.stringify({ error: 'Error al obtener partidos' })
            };
        }
        ...
        console.error('Error en misAsistencias:', error);
        return {
            statusCode: 500,
